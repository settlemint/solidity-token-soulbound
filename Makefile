...
.PHONY: build test format snapshot anvil deploy deploy-anvil subgraph cast help subgraph clear-anvil-port
...
deploy-anvil:
	...
	@forge create ./src/SoulboundToken.sol:Soulbound --rpc-url anvil --interactive --constructor-args "SoulBound" "SB" "ifps://" | tee deployment-anvil.txt
...
deploy-btp:
	...
	forge create ./src/SoulboundToken.sol:Soulbound $${EXTRA_ARGS} --rpc-url $${BTP_RPC_URL} $$args --constructor-args "SoulBound" "SB" "ifps://" | tee deployment.txt;
...
subgraph:
	...
	@cd subgraph && yq e '.specVersion = "0.0.4"' -i generated/solidity-token-soulbound.subgraph.yaml
	@cd subgraph && yq e '.description = "Solidity Soulbound Token"' -i generated/solidity-token-soulbound.subgraph.yaml
	@cd subgraph && yq e '.repository = "https://github.com/settlemint/solidity-token-soulbound"' -i generated/solidity-token-soulbound.subgraph.yaml
	@cd subgraph && yq e '.features = ["nonFatalErrors", "fullTextSearch", "ipfsOnEthereumContracts"]' -i generated/solidity-token-soulbound.subgraph.yaml
	@cd subgraph && npx graph codegen generated/solidity-token-soulbound.subgraph.yaml
	@cd subgraph && npx graph build generated/solidity-token-soulbound.subgraph.yaml
	@eval $$(curl -H "x-auth-token: $${BTP_SERVICE_TOKEN}" -s $${BTP_CLUSTER_MANAGER_URL}/ide/foundry/$${BTP_SCS_ID}/env | sed 's/^/export /'); \
	if [ -z "$${BTP_MIDDLEWARE}" ]; then \
		...
		npx graph deploy --version-label v1.0.$$(date +%s) --node $${BTP_MIDDLEWARE} --ipfs $${BTP_IPFS}/api/v0 $${BTP_SCS_NAME} generated/solidity-token-soulbound.subgraph.yaml; \
	fi
